        ...
        <tr>
            ...
            <td>                
                <?php foreach ($subItems as $p) { ?>   
                     <?php echo $this->tag->linkTo(array('items/artupal/' . $p->idArtupal, $p->idArtupal, 'class' => 'btn')); ?>
                <?php } ?>
            </td>      
        </tr>
    </tbody>
...
<?php echo $this->tag->linkTo(array('items/pdf/' . $item->idPalete . '?itemtype=palete', '<i class=\'icon-leaf icon-white\'></i>Get as PDF', 'class' => 'btn btn-danger', 'target' => '_blank')); ?>
